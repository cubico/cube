...
function createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1) {
	...
	foreach ($array as $categoryId => $category) {
 		if ($currentParent == $category['parent_id']) {						
 			...
 			if ($currLevel==$categoryId) $weight='bold';else $weight='normal';
 			echo '<span style="font-weight:'.$weight.'">'.$category['name'].'</span>';
...
if (!$hijos) echo ('no hi han dades per crear l\'arbre');
