				...
				<script type="text/javascript">
					var executeAction=function(action,domElem,elem){
						var checks=$('#batch_formAdgroup').serialize()
						var ok=true;
						...
						if (ok){ 
							if (checks.length>0){
								var form=$('#adgroupAdmgrpldapFormFilter');
				...
				<?php 
					...
					echo Session::getInstance()->getFlash('adgroupAdmgrpldapForm_info');
					...
					$nav_batch= Viewer::view('adgroup/list/actions/batch/admgrpldap',array('class'=>'batch_option'),$vars['viewtype']);
				?>	
					...
					if ($pagination)
					{
						...
						$nav=	Viewer::view('navigation/pagination',array('action'=>'filter', 
																		'offset' => $offset,
																		'filterName'=>'adgroupAdmgrpldapFormFilter',
						...
						$(document).ready(function(){
							...
							$('.list_action_link').click(function(){
								...
								form.submit();
										...
										<form style="" id="batch_formAdgroup">
					<table style="">
				    	<tr>
							<?php echo Viewer::view('adgroup/list/headers/admgrpldap',array('sort'=>$vars['sort']),$vars['viewtype_element']); ?>
						</tr>
				        <?php 
				        	$maxdata=count($vars['data']);
				        	if ($maxdata>0){
				            for($j=0;$j<$maxdata;$j++): ?>
							<?php $item=$vars['data'][$j]; //echo Viewer::object("adgroup",$vars['viewtype']); 
																
								if (isset($vars['selected']['adgroup.samaccountname'])
&& $vars['selected']['adgroup.samaccountname']==$item['samaccountname']
													)
									$class="selected";
								else 
							...
							<?php echo Viewer::view("adgroup/list/element/admgrpldap",
				    ...
				    <div  style="margin: 10px 0;">
						...
						//echo Viewer::view('adgroup/list/actions/buttons/admgrpldap',array(),$vars['viewtype']); 
					?>
					</div>
