$(document).ready(function () {
	...
	*/
	...
	if ($('#consolelogger').length){
		var display_debug_filter='block';
		$('#divlogger .link').live('click', function(){
			
			var c=$(this).attr('class').split(' ')
			display_debug_filter=(display_debug_filter=='block')?'none':'block'; //alternando
			$('#divlogger .row.item').not('.'+c[c.length-1]).css('display',display_debug_filter);
		});
		
		
		$('#divlogger .linkh').click(function(){
			var c=$(this).attr('class').split(' ');
			var classlogger=c[c.length-1];
			$('#divlogger .item').css('display','block');
			if (classlogger!='reset') $('#divlogger .item').not('.'+classlogger).css('display','none');
		});
		
		$('#consolelogger #buttonlogger').click(function(){
			var browser=$.browser;
			var display=$('#divlogger').css('display');
			$('#divlogger').slideToggle('fast');
			if (browser.msie && browser.version == '6.0') {
				if (display == 'none') $('select').hide();
				else $('select').show();
			}
		});
		
		$('#divlogger .menu div.model').click();
...
$.fn.elgg_topbardropdownmenu = function(options) {
  ...
  jQuery.fn.pulldownReadonly = function(){
      $(this).data('readonlyValues',$(this).find('option:selected'));
      $(this).unbind('change');
      $(this).bind('change',function(){
         var obj=$(this);
         var aa = obj.data('readonlyValues');
         obj.find('option:selected').removeAttr('selected');
         $(aa).each(function(i, j){
  ...
  jQuery.fn.disableForm = function(){
	...
	obj.find(':hidden').attr('disabled',false);
	obj.find('.required').attr('readonly',true).attr('disabled',false);
  }


  /* // manipular dom dentro de un iframe (http://codeimpossible.com/2010/03/04/using-jquery-on-an-iframe/)
  $.fn.extend({
     dom: function () {
         var $this = $(this);
