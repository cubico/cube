<?php
	class cube2Generator extends Generator
	{
		...
		public function generateTemplates($dir){
			...
			$templateList="/views/default/template/list.php";
			$templateForm="/views/default/template/form.php";
		...
		{
			$dir=real($this->ROOT."/apps/".$this->app."/modules/".$this->module);
			$generator=$this->generator;
			$module=$this->module;
			
			ob_start();$this->generateMethods();$content=ob_get_clean();
			$content=preg_replace("/\[\?php/","<?php",$content);
			$content=preg_replace("/\?\]/","?>",$content);
			...
			if (!file_exists(real($dir."/actions.class.php")))
			{
				ob_start();
				echo "<?php\n".
				"class ".ucfirst($module)."Actions extends Auto".ucfirst($generator)."Actions\n".
				"{\n".
		...
		public function generateListElement($headers=false)
		{
			...
			if ($headers){
				...
				if (!empty($refmodel['pks'])) {
					...
					$params="array('params'=>array('pks'=>".implode(".\"/\".",$strpks).",'entity'=>\$vars['entity']))";
			...
			foreach($this->columns as $numcolumn=>$column) // todas las columnas del render!
			{
				...
				if (isset($this->credentials[$column]))
			 	{
			 		$creden=array();
			 		//var_dump($this->credentials[$column]);
			 		foreach($this->credentials[$column] as $cred){
			 		...
			 		?>[?php if (<?php echo implode(" && ",$creden); ?>){ ?]<?php 
			 		$credentials=true;
			 	}
			...
			if (!empty($batch_options)){
				...
				<script type="text/javascript">
					var executeAction=function(action,domElem,elem){
						var checks=$('#batch_form<?php echo $this->referenceClass; ?>').serialize()
						var ok=true;
		...
		public function generateFormTemplate()
		{
			...
			$actions='<div class="minicontentWrapper">'.$actions.'</div>';
		...
		public function generateListTemplate()
		{
			?>[?php
				...
				$buttons='<div class="minicontentWrapper">'.$buttons.'</div>';
		...
		{
?>[?php
	class Auto<?php echo ucfirst($this->generator);?>Actions extends Actions
	{	
		...
		public function executeList($request)
		{
			...
			$filters=Viewer::view('<?php echo $this->model; ?>/list/filters/<?php echo $this->generator; ?>',array("values"=>$this->getFilters()),$this->VIEWTYPE);
			...
			$menus=Viewer::view('<?php echo $this->model; ?>/list/actions/menus/<?php echo $this->generator; ?>',array(),$this->VIEWTYPE).
						"<div id=\"owner_block\">".$filters."</div>";
			
			$buttons=Viewer::view('<?php echo $this->model; ?>/list/actions/buttons/<?php echo $this->generator; ?>',array(),$this->VIEWTYPE);
			
			$this->setView('list',array('params'=>$params,
										'filters'=>$filters,
										'menus'=>$menus,
		...
		public function executeNew($request)
		{
			...
			$this->titleMode="new";
			$this->form=Viewer::view("<?php echo $this->model; ?>/form/new/<?php echo $this->generator; ?>",
				array(	"values"=>$vars,
						"mode"=>"new",
						"viewtype"=>$this->VIEWTYPE,
			...
			$this->menus=Viewer::view("<?php echo $this->model; ?>/form/actions/menus/<?php echo $this->generator; ?>",array(),$this->VIEWTYPE);
		...
		{
			$form=$request->getFormObject('<?php echo $this->model; ?>/new/<?php echo $this->generator; ?>',$this->VIEWTYPE);
			$vars=$request->getFormVars('<?php echo $this->model; ?>/<?php echo $this->generator; ?>'); 
			$form->bind($vars);
			...
			if ($form->isValid())
			{
				...
				$error=$form->saveForm('<?php echo $this->referenceClass; ?>',$objects,$pks,true);
				
				if (!$error)
				{
					$request->setSelectedItem('<?php echo $this->model; ?>/<?php echo $this->generator; ?>',$pks);
					$ok=Viewer::_echo("form:savesuccess");
					$request->setFormVars("<?php echo $this->model; ?>/<?php echo $this->generator; ?>");
					$request->setError("<?php echo $this->model; ?>/<?php echo $this->generator; ?>",$ok,true,"reportedcontent_content archived_report");
					$this->redirect("<?php echo $this->module; ?>/edit/".implode("/",$pks));
				}
				//$this->redirect("<?php echo $this->module; ?>/list");
				
			}
			$request->setFormVars("<?php echo $this->model; ?>/<?php echo $this->generator; ?>");
			$request->setError("<?php echo $this->model; ?>/<?php echo $this->generator; ?>",$error,true,"reportedcontent_content active_report");
			$this->redirect("<?php echo $this->module; ?>/new");
		...
		public function executeEdit($request)
		{
			...
			$this->titleMode="edit";
			$this->form=Viewer::view("<?php echo $this->model; ?>/form/edit/<?php echo $this->generator; ?>",
				array(	"values"=>$vars,
						"mode"=>"edit",
						"viewtype"=>$this->VIEWTYPE,
		...
		{
			$form=$request->getFormObject('<?php echo $this->model; ?>/edit/<?php echo $this->generator; ?>',$this->VIEWTYPE);
			$vars=$request->getFormVars('<?php echo $this->model; ?>/<?php echo $this->generator; ?>'); 
			...
			if ($form->isValid())
			{
				...
				$pks=array();
				$objects=$form->getModelObjects();
				$error=$form->saveForm('<?php echo $this->referenceClass; ?>',$objects,$pks,false);
			...
			$request->setError("<?php echo $this->model; ?>/<?php echo $this->generator; ?>",$error,true,"reportedcontent_content active_report");
			$request->setFormVars("<?php echo $this->model; ?>/<?php echo $this->generator; ?>");
			$this->redirect('<?php echo $this->module ?>/edit');
